...
export default function BrandPage({ config, noticias, herramientas }: BrandPageProps) {
  return (
    <>
      ...
      <section className="relative overflow-hidden border-b border-zinc-200 bg-white">
        ...
        <Container className="relative py-12 sm:py-16">
          <div className="max-w-2xl space-y-4">
            <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs font-semibold ${config.pillClass}`}>
              <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-current" />
              {config.nombre}
            </span>
            <h1 className={`text-3xl font-black leading-tight tracking-tight text-zinc-900 sm:text-4xl`}>
              {config.nombre} <span className={config.textAccentClass}>en SaetaIA</span>
            </h1>
        ...
        <Container className="py-10">
          ...
          {noticias.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-zinc-200 bg-white py-16 text-center">
              <p className="text-zinc-500">No hay noticias todavía</p>
              <p className="mt-1 text-sm text-zinc-400">Vuelve pronto para ver las últimas novedades</p>
            </div>
          ) : (
      ...
      {/* ── Divider ───────────────────────────────────────────────────── */}
      <div className={`border-t-2 ${config.borderClass} mx-4 sm:mx-6 lg:mx-8 opacity-30`} />

      {/* ── Herramientas ──────────────────────────────────────────────── */}
      <section id="herramientas">
        <Container className="py-10">
          <div className="mb-6 space-y-1">
            <h2 className="text-xl font-bold text-zinc-900 sm:text-2xl">Herramientas Disponibles</h2>
            <p className="text-sm text-zinc-400">
