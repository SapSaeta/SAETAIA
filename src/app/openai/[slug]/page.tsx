...
function getNoticia(slug: string): Noticia | undefined {
  const { noticias } = getContenidoByBrand('openai')
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!noticia) return {}
  const canonicalUrl = `https://saetaia.com/openai/${noticia.slug}`
...
export default function OpenAISlugPage({ params }: PageProps) {
  const noticia = getNoticia(params.slug)
  if (!noticia) notFound()
  ...
  const jsonLd = {
    ...
    url: `https://saetaia.com/openai/${noticia.slug}`,
      ...
      <div className="mx-auto max-w-2xl space-y-8">
        ...
        {noticia.url_referencia && (
          <div className="border-t border-zinc-800 pt-6">
            ...
            <ExternalLink href={noticia.url_referencia}>Ver en OpenAI</ExternalLink>
          </div>
        )}
        <div className="border-t border-zinc-800 pt-6">
          <BackButton href="/openai" label="Volver a OpenAI" />
